...
<div class="container">
	...
	<div id="cls1" class="alert alert-info cls1" style="text-align:center">打印0.15元/页，累积打印页数最多可享受0.1元/页的超低价优惠！打印多份，可享受第二份0.08元/页无节操复印价！使用活动优惠码最高可享受免单福利! 【点我隐藏】  </div>
	<div id="cls2" class="alert alert-info cls2"  style="text-align:center">上传文件后记得设置文件页数！点击选择文件后别忘了点击旁边的上传。登陆用户记点击我的优惠码！【点我隐藏】  </div>
	...
	<form class="form-horizontal" action="" method="post">
		...
		<script id="tr-template" type="text/x-jquery-tmpl">
			...
			<tr>
				...
				<td style="width:5%;text-align:center" class="goods"><label ><span class="table-icon-preview"><i class="fa fa-file-word-o text-primary"></span></label></td>
				<td style="width:30%;text-align:center" class="goods"><label>{{= filesname}}</label></td>
				<td style="width:10%;text-align:center" class="goods"><label ><input type="text"
					class="number form-control input-sm" id="pages" name="pages" value="0" /></label></td>
				...
				<td  style="display:none"><input id="fileid" name="fileid" type="text" hidden="hidden" value="{{= uid}}">
		...
		<div class="row">
			<div class="col-md-12 col-lg-12 col-sm-12">
				<div style="border-top:1px solid gray;padding:4px 10px;">
					<c:if test="${currentUser.uid ne null}">
						<div class="col-md-12 col-lg-12 col-sm-12">
		...
		<button class="btn btn-primary" name="submit" id="submit"
			style="text-align: center">下一步</button>
	</form>
...
<script>
	...
	function start(){		
		...
		function getSubTotal(row) {
		    ...
		    var result = price +(qty -1)*price/0.15 * sec; //计算金额小计
	...
	$(".clicks").bind("click",function(){
		$.get("<%=basePath%>option/getbonus",function(msg){
			...
			$("#bonuscount").text(msg.bonus_count);
		});
	});
