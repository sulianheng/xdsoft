...
public class OrdersService {
		...
		if (user != null && user.containsKey("ordernum_id")) {
			...
			if (user.containsKey("id")) {
				...
				if (user.containsKey("bonus")) {
					...
					if (calc >= 0) {
						...
						this.InsertOrderList(
								...
								userService.addUserAddressInfo(user.get("id")
										...
										calcresult.get("BonusPages")
										.toString(), 
								df.format(new Double((Integer
										.parseInt(calcresult.get("payPages")
												.toString()))* 0.15
										)),
								"0"
								);
						...
						user.put("youhui", calcresult.get("PrintPages")
								.toString());
						user.put("price", df.format(new Double((Integer
								.parseInt(calcresult.get("payPages")
										.toString()))* 0.15
								)));
					} else {
						// @Test
						System.out.println("calc  + " + calc + "      " + Integer.parseInt(calcresult.get("ppPages")
								.toString()));
						System.out.println(df.format(new Double(
								(0 - calc) * 0.15)));
						// 用户实际支付
						String price = df.format(new Double((Integer
								.parseInt(calcresult.get("copyPages")
										.toString()))
								* 0.15
								+ Integer.parseInt(calcresult.get("ppPages")
										.toString()) * 0.08));
						// 更新用户优惠额度
						robOrderService.updateBonusCount(
						...
						this.InsertOrderList(
								...
								userService.addUserAddressInfo(user.get("id")
										...
										address_info_id, dorm, tel),
										user.get("id").toString(),
										user.get("bonus").toString(), price,"0"
								);
						// 优惠信息
					...
					// 计算金额
					String price =df.format(new Double((Integer
							.parseInt(calcresult.get("ppPages")
									.toString()))
							* 0.15
							+ Integer.parseInt(calcresult.get("copyPages")
									.toString()) * 0.08));
					// 用户未使用打印优惠
					this.InsertOrderList(user.get("ordernum_id") + "", userService
							.addUserAddressInfo(user.get("id")
									.toString(), recieve_name,
									address_info_id, dorm, tel),
							user.get("id").toString(), "0", price, "0");
					// 打印总页数
					user.put("countpages", calcresult.get("PrintPages"));
					// 用户实际支付
					user.put("price", price);
	...
	public Map<String, Object> CalcPages(String Ordernum, String bonus) {
		...
		result.put("ppPages", calcAllPages-copyPages);
		if (Integer.parseInt(bonus) > calcAllPages)
			{
			result.put("restBonusPages", Integer.parseInt(bonus) - calcAllPages);
			result.put("BonusPages", calcAllPages);
			result.put("payPages", 0);
			}
		else
			{
			result.put("restBonusPages", 0);
			result.put("BonusPages", calcAllPages - Integer.parseInt(bonus));
			result.put("payPages", Integer.parseInt(bonus));
			}
		result.put("PrintPages", calcAllPages);
		
			result.put("copyPages", copyPages );

		System.out.println("copy   pages   " + (copyPages ));
		return result;
	...
	public List<Map<String, Object>> queryFilesInfoByOrderlist_id(
			Integer id) {
		return ordersMapper.QueryFilesInfoByOrdernum_id(id);
	}
